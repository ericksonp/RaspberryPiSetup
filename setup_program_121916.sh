...
cd ~
sudo apt-get --yes --force-yes install i2c-tools libi2c-dev python-dev
...
#setup the neopixels
cd ~
sudo apt-get --yes --force-yes install build-essential python-dev git scons swig
...
cd ~
sudo apt-get install --yes --force-yes build-essential libi2c-dev i2c-tools python-dev libffi-dev
